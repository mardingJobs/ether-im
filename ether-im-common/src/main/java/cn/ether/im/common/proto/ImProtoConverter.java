package cn.ether.im.common.proto;
...
public class ImProtoConverter {
    ...
    public static ImTextProto encodeToTextProto(ImInfo imInfo) throws ImProtocException {
        ...
        try {
            ...
            if (imInfo.getType() == ImInfoType.INFO) {
                imMessageTextProto.setContent(imInfo.getInfo());
                ...
                ImMessage message = (ImMessage) imInfo;
                imMessageTextProto.setMessageId(message.getId());
            } else if (imInfo.getType() == ImInfoType.MESSAGE_RECEIVED) {
                ImMessage message = (ImMessage) imInfo;
                imMessageTextProto.setMessageId(message.getId());
            }
        } catch (Exception e) {
    ...
    public static ImInfo decodeToImInfo(ImTextProto imTextProto) throws ImProtocException {
        ...
        if (infoType == null) {
        ...
        } else if (infoType == ImInfoType.INFO) {
            ImInfo imInfo = new ImInfo(infoType);
            ...
            imMessage.setType(ImInfoType.MESSAGE);
